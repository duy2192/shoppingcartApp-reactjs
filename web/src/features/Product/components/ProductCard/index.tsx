...
export default function ProductCart({ product, loading }: IProductCardProps) {
  ...
  return (
    <div className="flex font-sans rounded-md overflow-hidden h-60 shadow-md w-full bg-white relative">
      ...
      <div
        className={classNames('flex-none relative cursor-pointer w-1/2', { 'w-full': loading })}
        onClick={handleProductClick}
      >
        <img
          src={thumbnails[0]}
          alt=""
          className={classNames('absolute inset-0 w-60 h-auto p-4 select-none block', {
            hidden: loading,
          })}
          loading="lazy"
        />
        <div
          role="status"
          className={classNames(' animate-pulse space-x-8  items-center flex transition-all ', {
            hidden: !loading,
          })}
        >
          <div className="flex justify-center items-center h-60 bg-gray-300 rounded dark:bg-gray-700 w-full">
            <svg
      ...
      <form className={classNames('flex-auto p-6 ')}>
        <div className="flex flex-wrap">
          ...
          <div className="text-lg font-semibold text-slate-700 flex flex-col h-14">
            {salePrice && <span className=" line-through text-md text-slate-500">{formatPrice(price)}</span>}
