...
import { IProvince, ProvinceInfo, ProvinceParams } from 'models';
...
export const getProvinceList = (payload?: ProvinceParams): IProvince => {
  const data:IProvince = provinces;
    return {
      city: data.city,
      district: data?.district?.filter((item:ProvinceInfo) => (payload?.city ? item.city === payload?.city : true)),
      ward: data?.ward?.filter((item:ProvinceInfo) => (payload?.city ? item.city === payload?.city : true)&&(payload?.district ? item.district === payload?.district : true)),
    };
};
